...
exports.author_create_post = [
	...
	author.save((err) => {
	  if (err) return next(err)
	  res.redirect(author.url);    // Successful - redirect to new author record.
	})
      }
    }
];
...
exports.author_update_get = (req, res, next) => {
  ...
  async.parallel({
    ...
    if (results.author == null) {   // No result
      const authorErr = new Error('Author not found');
      authorErr.status = 404
...
exports.author_update_post = [
	...
	Author.findByIdAndUpdate(req.params.id, author, {}, (err, updatedAuthor) => {
	  if (err) return next(err)
	  res.redirect(updatedAuthor.url);    // Successful - redirect to new author record.
	})
      }
    }
]
